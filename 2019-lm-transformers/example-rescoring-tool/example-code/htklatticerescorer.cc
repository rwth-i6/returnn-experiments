...
void HtkLatticeRescorer::Reset() {
  ...
  if (is_dependent_)
    nodes_.clear();
  if (nodes_.empty()) {
    InitStateVars(session_, state_vars_assign_ops_, state_vars_assign_inputs_, state_vars_size_, state_num_heads_);
...
void HtkLatticeRescorer::RescoreLattice() {
  ...
  for (const Node *const node : sorted_nodes_) {
    ...
    while (!node_hypotheses.empty()) {
      const Hypothesis &hypothesis = node_hypotheses.top();
      for (const int link_id : successor_links_[node->id]) {
        // Sets the current value of state variables.
        TF_SetState(session_, state_vars_assign_ops_, state_vars_assign_inputs_, state_vars_size_,
                    state_num_heads_, hypothesis.state, hypothesis.history_word_index);
        const Link &link = links_[link_id];
        ...
        if (link.lm_score == 0.) {
          ...
          new_hypothesis.history_word_index = hypothesis.history_word_index;
        } else {
...
// Initialize the state variables with zeros.
void HtkLatticeRescorer::InitStateVars(tensorflow::Session* session,
                                       ...
                                       const std::vector<int> state_num_heads) {
  ...
  for (int i=0; i < state_vars_assign_ops.size(); i++) {
    tensorflow::TensorShape shape = {1, state_num_heads[i], 0, state_vars_size[i]};
    tensorflow::Input::Initializer zeros(0.0f, shape);
...
void HtkLatticeRescorer::TF_SetState(tensorflow::Session *session,
                                     const std::vector<std::string> state_vars_assign_ops,
                                     const std::vector<std::string> state_vars_assign_inputs,
                                     const std::vector<int> state_vars_size,
                                     const std::vector<int> state_num_heads,
  ...
  std::string var_assign_op_name, var_assign_op_input_name;
  for (int i=0; i < state_vars_assign_ops.size(); i++) {
    tensorflow::TensorShape shape = {1, state_num_heads[i], position, state_vars_size[i]};
    tensorflow::Input::Initializer keep_state_var(0.0f, shape);
    var_assign_op_name = state_vars_assign_ops[i];
    var_assign_op_input_name = state_vars_assign_inputs[i];
...
// Extract the current states and store them in the hypothesis.
void HtkLatticeRescorer::TF_ExtractState(tensorflow::Session* session,
  ...
  for (int i=0; i < state_vars.size(); i++) {
    ...
    lstm_state.resize(outputs[0].dim_size(1) * outputs[0].dim_size(2) * outputs[0].dim_size(3));
...
float HtkLatticeRescorer::TF_ComputeLogProbability(tensorflow::Session* session,
  ...
  tensorflow::Tensor position = tensorflow::Tensor(tensorflow::DT_INT32, tensorflow::TensorShape({}));
  epoch_step.scalar<tensorflow::int64>()() = word_index;
  position.scalar<tensorflow::int32>()() = word_index;
  std::vector<tensorflow::Tensor> outputs;
  std::vector<std::pair<std::string, tensorflow::Tensor>> inputs;
  inputs = {{tensor_names[0], delayed_dim0_size},
            {tensor_names[1], in_word},
            {tensor_names[2], epoch_step},
            {tensor_names[5], position}};  // Positional encoding.
