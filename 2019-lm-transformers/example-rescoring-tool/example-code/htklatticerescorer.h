...
public:
  ...
  HtkLatticeRescorer(const ConstVocabularyPointer &vocabulary,
      ...
      : Rescorer(vocabulary, num_oov_words, nn_lambda),
        ...
        state_num_heads_(state_num_heads),
...
private:
  ...
  // state_vars: the names of state variables
  ...
  void TF_SetState(tensorflow::Session* session, const std::vector<std::string> state_vars_assign_ops,
                   const std::vector<std::string> state_vars_assign_inputs,
                   const std::vector<int> state_vars_size, const std::vector<int> state_num_heads,
                   const State &state, const int position);
  // Extract the current value of state variables and store them in the current values of the new hypothesis
  // state_vars: the names of state variables
  ...
  float TF_ComputeLogProbability(tensorflow::Session* session, const std::vector<std::string> tensor_names,
                                 const int history_word, const int target_word, const int word_index);
  ...
  std::vector<int> state_vars_size_;
  std::vector<int> state_num_heads_;
