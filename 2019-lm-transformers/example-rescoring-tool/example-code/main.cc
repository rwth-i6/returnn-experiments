...
void StateVars(const std::string path_to_state_vars_list,
               std::vector<std::string>& state_vars,
               std::vector<std::string>& state_vars_assign_ops,
               std::vector<std::string>& state_vars_assign_inputs,
               std::vector<int>& state_vars_size,
               std::vector<int>& state_vars_num_heads) {
  std::cout << "Reading state names from: " << path_to_state_vars_list << std::endl;
  ...
  if(!infile.eof()){
    ...
    while(line.length() != 0) {
      ...
      stringin >> state_var_item;
      state_vars_num_heads.push_back(atoi(state_var_item.c_str()));
      stringin >> state_var_item;
...
void EvaluateCommandLine(const po::variables_map &options) {
  try {
    ...
    std::vector<int> state_vars_size;
    std::vector<int> state_num_heads;
    StateVars(path_to_state_vars_list, state_vars, state_vars_assign_ops, state_vars_assign_inputs, state_vars_size, state_num_heads);
