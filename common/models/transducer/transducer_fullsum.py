...
def _get_network_full_sum(*, epoch0: int, task: str):
  ...
  net_dict = _get_network(
    target=_target if task == "train" else "targetb")
  net_dict["#copy_param_mode"] = "subset"
  return net_dict


def _get_network(*, task: str, target: str):
  lstm_dim = 1024
  l2 = 0.0001
  ...
  def get_output_dict(train, search, target=target, beam_size=beam_size):
      return {
          "class": "rec",
          "from": "encoder",
          "include_eos": True,
